    ...
    <div className="space-y-4">
      ...
      {creating && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-5 space-y-4">
          <h3 className="font-semibold text-green-900">New subscription</h3>
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={newForm.planId} onChange={(e) => setNewForm((f) => ({ ...f, planId: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">
                {plans.map((p) => <option key={p.id} value={p.id}>{p.name} — {p.price.toLocaleString("en-US")} {p.currency}</option>)}
              </select>
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <input value={newForm.notes} onChange={(e) => setNewForm((f) => ({ ...f, notes: e.target.value }))}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="e.g. paid in cash..." />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setCreating(false)} className="px-4 py-2 border border-gray-300 rounded-xl text-sm text-gray-700 hover:bg-gray-50"><X size={14} className="inline mr-1" />Cancel</button>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {subs.length === 0 && (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-gray-400">No subscriptions yet</td></tr>
            )}
            {subs.map((sub) => (
                ...
                <td className="px-5 py-3">
                  {editing?.id === sub.id ? (
                    ...
                    <span className="text-gray-500 text-xs">
                      {sub.endDate ? format(new Date(sub.endDate as string), "d MMM yyyy", { locale: enUS }) : "No end date"}
                    </span>
                  )}
                </td>
                <td className="px-5 py-3">
                  <div className="flex items-center justify-end gap-1">
                    {editing?.id === sub.id ? (
                      ...
                      <>
                        ...
                        <button onClick={() => deleteSub(sub.id)} className="text-xs bg-red-600 text-white px-2 py-1 rounded-lg">{loading ? <Loader2 size={10} className="animate-spin" /> : "Yes"}</button>
